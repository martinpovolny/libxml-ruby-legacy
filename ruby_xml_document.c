/* $Id: ruby_xml_document.c,v 1.4 2004/02/27 09:42:42 xpovolny Exp $ */
...
ruby_xml_document_encoding_get(VALUE self) {
  ...
  if (rxd->doc->encoding == NULL)
    ...
    return(rb_str_new2((char *)rxd->doc->encoding));
}
...
ruby_xml_document_encoding_set(VALUE self, VALUE encoding) {
  ...
  Data_Get_Struct(self, ruby_xml_document, rxd);
  rxd->doc->encoding = (unsigned char *)ruby_strdup(STR2CSTR(encoding));
...
ruby_xml_document_new2(VALUE class, VALUE xmlver) {
  ...
  rxd->data = NULL;
  rxd->data_type = RUBY_LIBXML_SRC_TYPE_NULL;
  rxd->doc = xmlNewDoc((unsigned char *)STR2CSTR(xmlver));
  rxd->is_ptr = 0;
  rxd->xmlver = xmlver;
...
ruby_xml_document_save(int argc, VALUE *argv, VALUE self) {
  ...
  filename = (unsigned char *)STR2CSTR(argv[0]);

  Data_Get_Struct(self, ruby_xml_document, rxd);
  len = xmlSaveFormatFileEnc((char *)filename, rxd->doc, (char *)rxd->doc->encoding, format);
  if (len == -1)
...
ruby_xml_document_to_s(int argc, VALUE *argv, VALUE self) {
  ...
  if (rxd->doc == NULL) {
    ...
    if (format) {
      xmlDocDumpFormatMemoryEnc(rxd->doc, &result, &len,
				(char *)rxd->doc->encoding, format);
    } else {
      xmlDocDumpMemoryEnc(rxd->doc, &result, &len,
			  (char *)rxd->doc->encoding);
    }
  } else {
    if (format)
  ...
  return(rb_str_new2((char *)result));
}
...
ruby_xml_document_url_get(VALUE self) {
  ...
  if (rxd->doc->URL == NULL)
    ...
    return(rb_str_new2((char *)rxd->doc->URL));
}
...
ruby_xml_document_version_get(VALUE self) {
  ...
  if (rxd->doc->version == NULL)
    ...
    return(rb_str_new2((char *)rxd->doc->version));
}
